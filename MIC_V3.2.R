...
timepoint_oddPlate <- data.frame(matrix(unlist(tpODDPlates), nrow = num_plates/2 , byrow = T), stringsAsFactors = FALSE)
colnames(timepoint_oddPlate) <- c("TIME","DATE")
timepoint_evenPlate <- data.frame(matrix(unlist(tpEVENPlates), nrow =num_plates/2, byrow = T), stringsAsFactors = FALSE)
